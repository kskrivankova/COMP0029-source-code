...
class NetworkFloodingSim(Network):
    ...
    def process_recv(self, data, meta, *args, **kwargs):
        ...
        if (self.is_consistent(flagged_attributes)) and (version_id == self.version_id):
            ...
            if (version_id > self.version_id) or (not self.versioning):
                self.update_default_data([list(layer.values()) for layer in data], version_id)
    ...
    def scheduler_callback(self, *args, **kwargs):
        ...
        # TODO CHange for hardware
        self.scheduler_thread = _thread.start_new_thread(listen, tuple())
    ...
    def reset_interval(self):
        self.interval_alarm.cancel()
        self.transmit_alarm.cancel()
    ...
    def reset_primitives(self):
        ...
        self.interval_alarm = Timer.Alarm(
            self.restart_interval, self.interval_length, periodic=False, arg=None
        )
        self.transmit_alarm = self.interval_alarm = Timer.Alarm(
            self.uplink, self.interval_length, periodic=False, arg=None
        )

    def uplink(self):
        if self.counter < self.REDUNDANCY_CONST:
            # TODO: Change this!
            self.channel.send(self.default_data)
